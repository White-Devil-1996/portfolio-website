...
(function() {
  ...
  function initSwiper() {
    if (typeof Swiper !== 'undefined') {
      document.querySelectorAll(".init-swiper").forEach(function(swiperElement) {
        // Try to get config from script tag first
        let config = null;
        const scriptTag = swiperElement.querySelector(".swiper-config");
        
        if (scriptTag && scriptTag.innerHTML) {
          try {
            config = JSON.parse(scriptTag.innerHTML.trim());
          } catch (e) {
            console.warn('Failed to parse Swiper config:', e);
            config = getDefaultSwiperConfig();
          }
        } else {
   ...
   */
  function getDefaultSwiperConfig() {
    return {
      loop: true,
      speed: 600,
      autoplay: {
